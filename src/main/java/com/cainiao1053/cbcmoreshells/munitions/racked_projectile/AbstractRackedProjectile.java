...
public abstract class AbstractRackedProjectile extends AbstractCannonProjectile {

	private static final EntityDataAccessor<ItemStack> TRACER = SynchedEntityData.defineId(AbstractRackedProjectile.class, EntityDataSerializers.ITEM_STACK);

	protected int age = 0;
	protected int maxAge;
	protected double elasticity = 1.7;

	protected AbstractRackedProjectile(EntityType<? extends AbstractRackedProjectile> type, Level level) {
		super(type, level);
	}
...
//						}, radius));
	...
	protected ImpactResult calculateBlockPenetration(ProjectileContext projectileContext, BlockState state, BlockHitResult blockHitResult) {
		...
		if (!this.level().isClientSide) {
			...
			if (bounced) {
				...
				effectNormal = curVel.subtract(normal.scale(normal.dot(curVel) * this.elasticity));
			} else {
	...
	public void setLifetime(int lifetime) { this.maxAge = lifetime; }
	...
	public float addedChargePower() { return this.getRackedProjectileProperties().addedChargePower(); }
	public float minimumChargePower() { return this.getRackedProjectileProperties().minimumChargePower(); }
	public boolean canSquib() { return this.getRackedProjectileProperties().canSquib(); }
