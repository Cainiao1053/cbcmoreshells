...
public abstract class AbstractCannonTorpedoProjectile extends AbstractTorpedoProjectile {
	...
	@Override
	protected Vec3 getForces(Vec3 position, Vec3 velocity, boolean inWarer) {
		Vec3 parentForces = Vec3.ZERO;
		double gravityForce = this.getGravity();// 调用父类方法
		return parentForces.add(0.0d, gravityForce, 0.0d); // 例如给X方向增加一个额外的力
	}
	...
	protected ImpactResult calculateBlockPenetration(ProjectileContext projectileContext, BlockState state, BlockHitResult blockHitResult) {
		...
		Vec3 accel = this.getForces(this.position(), this.getDeltaMovement(), false);
